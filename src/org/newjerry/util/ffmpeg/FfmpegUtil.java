...
import org.newjerry.util.ffmpeg.domain.ImageMetaInfo;
import org.newjerry.util.ffmpeg.domain.MusicMetaInfo;
import org.newjerry.util.ffmpeg.domain.VideoMetaInfo;
import org.newjerry.util.ffmpeg.gif.AnimatedGifEncoder;
...
/**
 * 基于FFmpeg内核来编解码音视频信息； 使用前需手动在运行环境中安装FFmpeg运行程序，然后正确设置FFmpeg运行路径后MediaUtil.
 * java才能正常调用到FFmpeg程序去处理音视频；
 * 
 * Author: dreamer-1
 * 
 * version: 1.0
...
public class FfmpegUtil {
	 ...
	 * 可以处理的视频格式
	 */
    private final static String[] VIDEO_TYPE = { "MP4", "WMV","MKV" };
	/**
	 * 可以处理的图片格式
	 */
    private final static String[] IMAGE_TYPE = { "JPG", "JPEG", "PNG", "GIF" };
	/**
	 * 可以处理的音频格式
	 */
    private final static String[] AUDIO_TYPE = { "AAC" };

	/**
	 * 视频帧抽取时的默认时间点，第1200s（秒） （Time类构造参数的单位:ms）
	 */
	@SuppressWarnings("deprecation")
	private static final Time DEFAULT_TIME = new Time(0, 0, 12000);
    /**
	 * 视频帧抽取的默认时长，单位：s（秒）
	 */
	 ...
	 */
	private static  String FFMPEG_PATH = System.getProperty("catalina.home")+ "\\webapps\\familyCinema\\ffmpeg.exe"; // /usr/bin/ffmpeg
	...
	public static String executeCommand(List<String> commonds) {
		...
		try {
			...
			log.info("--- 已执行的FFmepg命令： ---" + cmdStr + " 已执行完毕,执行结果： "
					+ resultStr);
			return result;
	 ...
	 * @param crf
	 *            指定视频的质量系数（值越小，视频质量越高，体积越大；该系数取值为0-51，直接影响视频码率大小）,取值参考：
	 *            CrfValueEnum.code
	 * @param preset
	 ...
	 *            视频高度；为空则保持源视频高度
	 */
	public static void convertVideo(File fileInput, File fileOutPut,
			boolean withAudio, Integer crf, String preset, Integer width,
			Integer height) {
	...
	/**
	 * 视频帧抽取 默认抽取第10秒的帧画面 抽取的帧图片默认宽度为300px
	 * 
	...
	/**
	 * 视频帧抽取（抽取指定时间点的帧画面） 抽取的视频帧图片宽度默认为320px
	 * 
	...
	/**
	 * 视频帧抽取（抽取指定时间点、指定宽度值的帧画面） 只需指定视频帧的宽度，高度随宽度自动计算
	 * 
	 * 转换后的文件路径以.gif结尾时，默认截取从指定时间点开始，后10s以内的帧画面来生成gif
	 * 
	 * @param videoFile
	...
	public static void cutVideoFrame(File videoFile, File fileOutPut,
		...
		if (!"GIF".equals(StringUtils.upperCase(format))) {
			...
			cutVideoFrame(videoFile, fileOutPutPath, time, width, height, 1,
                    false);
		} else {
			...
			try {
				cutVideoFrame(videoFile, tempPath, time, width, height,
						DEFAULT_TIME_LENGTH, true);
				// 生成gif
				String images[] = file.list();
				for (int i = 0; i < images.length; i++) {
					images[i] = tempPath + File.separator + images[i];
				}
				createGifImage(images, fileOutPut.getAbsolutePath(),
						DEFAULT_GIF_PLAYTIME);
			} catch (Exception e) {
				...
				String images[] = file.list();
				for (int i = 0; i < images.length; i++) {
					File fileDelete = new File(tempPath + File.separator
							+ images[i]);
					fileDelete.delete();
				}
				file.delete();
	 ...
	 *            截取的视频帧图片的宽度（单位：px）
	 * @param height
	 *            截取的视频帧图片的高度（单位：px，需要大于20）
	 * @param timeLength
	 *            截取的视频帧的时长（从time开始算，单位:s，需小于源视频的最大时长）
	 * @param isContinuty
	 *            false - 静态图（只截取time时间点的那一帧图片），true -
	 *            动态图（截取从time时间点开始,timelength这段时间内的多张帧图）
	 */
	private static void cutVideoFrame(File videoFile, String path, Time time,
			int width, int height, int timeLength, boolean isContinuty) {
		...
		if (time.getTime() + timeLength > info.getDuration()) {
			throw new RuntimeException("开始截取视频帧的时间点不合法：" + time.toString()
					+ "，因为截取时间点晚于视频的最后时间点");
		}
	 ...
	 *            开始抽取的时间点（单位:s）
	 * @param timeLength
	 *            需要抽取的时间段（单位:s，需小于源视频最大时长）；例如：
	 *            该参数值为10时即抽取从startTime开始之后10秒内的视频作为新视频
	 */
	public static void cutVideo(File videoFile, File outputFile,
		...
		if (startTime.getTime() + timeLength > info.getDuration()) {
			throw new RuntimeException("截取时间不合法：" + startTime.toString()
					+ "，因为截取时间大于视频的时长");
		}
		try {
	 ...
	 * 解析视频的基本信息（从文件中）
	 * 
	 * 解析出的视频信息一般为以下格式： Input #0, mov,mp4,m4a,3gp,3g2,mj2, from '6.mp4':
	 * Duration: 00:00:30.04, start: 0.000000, bitrate: 19031 kb/s Stream
	 * #0:0(eng): Video: h264 (Main) (avc1 / 0x31637661), yuv420p(tv, bt709),
	 * 1920x1080, 18684 kb/s, 25 fps, 25 tbr, 25k tbn, 50 tbc (default) Stream
	 * #0:1(eng): Audio: aac (LC) (mp4a / 0x6134706D), 48000 Hz, stereo, fltp,
	 * 317 kb/s (default)
	 * 
	 * 注解： Duration: 00:00:30.04【视频时长】, start: 0.000000【视频开始时间】, bitrate: 19031
	 * kb/s【视频比特率/码率】 Stream #0:0(eng): Video: h264【视频编码格式】 (Main) (avc1 /
	 * 0x31637661), yuv420p(tv, bt709), 1920x1080【视频分辨率，宽x高】, 18684【视频比特率】 kb/s,
	 * 25【视频帧率】 fps, 25 tbr, 25k tbn, 50 tbc (default) Stream #0:1(eng): Audio:
	 * aac【音频格式】 (LC) (mp4a / 0x6134706D), 48000【音频采样率】 Hz, stereo, fltp,
	 * 317【音频码率】 kb/s (default)
	 * 
	 * @param videoFile
	 *            源视频路径
	...
	public static VideoMetaInfo getVideoMetaInfo(File videoFile) {
		...
		Matcher videoMusicStreamMacher = musicStreamPattern
				.matcher(parseResult);
		...
		try {
			...
			if (videoMusicStreamMacher.find()) {
				...
				musicBitrate = Integer
						.parseInt(videoMusicStreamMacher.group(6)); // 提取比特率
			}
		} catch (Exception e) {
	...
	public static ImageMetaInfo getImageInfo(InputStream inputStream) {
		...
		try {
			...
			imageInfo.setSize(Long.valueOf(String.valueOf(inputStream
					.available())));
			return imageInfo;
		} catch (Exception e) {
	...
	public static String getMetaInfoFromFFmpeg(File inputFile) {
		...
		String executeResult = FfmpegUtil.executeCommand(commond);
	 ...
	 *            播放的延迟时间，可调整gif的播放速度
	 */
	private static void createGifImage(String image[], String outputPath,
			int playTime) {
		if (null == outputPath) {
